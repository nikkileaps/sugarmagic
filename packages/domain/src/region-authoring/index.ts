...
/**
 * A declared scatter group (Plan 070.3, #349) — a DERIVED view over brushed
 * placed assets, NOT a persisted entity. Open question 3 is resolved as
 * DERIVE, not persist: every field is computable from `placedAssets`, so the
 * region document is unchanged (the round-trip is trivially invisible — no
 * migration, no alias to keep in lockstep) and old regions "have" groups the
 * moment this ships.
 *
 * Grouping keys on FOLDER IDENTITY (`parentFolderId`), never on "patch"-ness:
 * a folder carries no provenance marker — the "<Asset> patch" name is UI-only
 * (see [[displayname-vs-identity]]), so patch detection would be a
 * displayName-as-semantic-key bug. Folderless brushed instances (reachable:
 * folder deletion reparents members to null, MovePlacedAssetToFolder relocates
 * them) fall back to one synthetic group per assetDefinitionId, making the
 * derivation TOTAL over the schema. Hand-placed (non-`brushed`) instances are
 * never grouped. Scope: base `region.placedAssets` only — Scene-overlay
 * brushed placements are out of scope for 070.3 (documented seam).
 */
export interface ScatterGroup {
  /** Stable id: the backing folderId, or `scatter:<assetDefinitionId>` for
   *  the folderless synthetic bucket. */
  groupId: string;
  displayName: string;
  /** The asset every member shares, or null for a mixed folder. */
  assetDefinitionId: string | null;
  memberInstanceIds: string[];
  /** true = grouped by a real folder; false = synthetic folderless bucket. */
  folderBacked: boolean;
}

const SCATTER_GROUP_SYNTHETIC_PREFIX = "scatter:";

function homogeneousAssetDefinitionId(
  members: PlacedAssetInstance[]
): string | null {
  const first = members[0]?.assetDefinitionId ?? null;
  return members.every((m) => m.assetDefinitionId === first) ? first : null;
}

export function resolveScatterGroups(region: RegionDocument): ScatterGroup[] {
  const folders = new Map(
    (region.folders ?? []).map((f) => [f.folderId, f])
  );
  const byFolder = new Map<string, PlacedAssetInstance[]>();
  const folderless = new Map<string, PlacedAssetInstance[]>();
  for (const asset of region.placedAssets ?? []) {
    if (asset.brushed !== true) {
      continue;
    }
    // A dangling parentFolderId (folder deleted) falls into the folderless
    // bucket too, so the derivation can't strand a brushed instance.
    if (asset.parentFolderId && folders.has(asset.parentFolderId)) {
      const list = byFolder.get(asset.parentFolderId);
      if (list) list.push(asset);
      else byFolder.set(asset.parentFolderId, [asset]);
    } else {
      const list = folderless.get(asset.assetDefinitionId);
      if (list) list.push(asset);
      else folderless.set(asset.assetDefinitionId, [asset]);
    }
  }
  ...
  for (const [folderId, members] of byFolder) {
    groups.push({
      groupId: folderId,
      displayName: folders.get(folderId)?.displayName ?? "Scatter",
      assetDefinitionId: homogeneousAssetDefinitionId(members),
      memberInstanceIds: members.map((m) => m.instanceId),
      folderBacked: true
    });
  }
  for (const [assetDefinitionId, members] of folderless) {
    groups.push({
      groupId: `${SCATTER_GROUP_SYNTHETIC_PREFIX}${assetDefinitionId}`,
      displayName: members[0]?.displayName ?? "Scattered",
      assetDefinitionId,
      memberInstanceIds: members.map((m) => m.instanceId),
      folderBacked: false
    });
