...
describe("plugin infrastructure", () => {
  ...
  it("generates .github/workflows/sugardeploy-deploy.yml when at least one hosted target is configured (story 46.7)", () => {
    ...
    // 46.7 → v1; 46.10 → v2 (env-var injection); 46.10 → v3
    // (dropped auth@v2 service_account:); 46.10 → v4 (frontend
    // job switched to thin checkout-and-netlify-deploy off the
    // pre-baked .sugarmagic/published-web/ directory).
    expect(yaml).toContain("# SUGARMAGIC WORKFLOW TEMPLATE VERSION: 4");
    expect(parseWorkflowTemplateVersionStamp(yaml)).toBe(4);
