/**
 * packages/plugins/src/catalog/sugarlang/tests/data/function-inventory.test.ts
 *
 * Purpose: Pins the function inventory JSON schema, the es seed data, and the runtime loader.
 *
 * Exports:
 *   - none
 *
 * Relationships:
 *   - Depends on ../../data/schemas/function-inventory.schema.json and
 *     ../../data/languages/es/function-inventory.json as checked-in source of truth.
 *   - Exercises ../../runtime/inventory/function-inventory-loader.
...
import esInventory from "../../data/languages/es/function-inventory.json";
import functionInventorySchema from "../../data/schemas/function-inventory.schema.json";
import {
  FunctionInventoryLoader,
  buildInterpretLexiconFromInventory,
  getAllInventoryChunks,
  loadFunctionInventory
} from "../../runtime/inventory/function-inventory-loader";
import { INTERPRET_LEXICON_CATEGORIES } from "../../runtime/contracts/function-inventory";

describe("function-inventory schema + data", () => {
  const ajv = new Ajv2020({ strict: true });

  it("es inventory validates against the schema", () => {
    const validate = ajv.compile(functionInventorySchema);
  ...
  it("es inventory has the required item-zero meta-language function", () => {
    const metaFn = esInventory.functions.find(
      (fn) => fn.functionId === "meta-language"
    );
  ...
  it("es inventory includes the four interpretLexicon category functions", () => {
    const categories = esInventory.functions
  ...
  it("every chunk has at least one surfaceForm and one constituentLemma", () => {
    for (const fn of esInventory.functions) {
      for (const [lang, chunks] of Object.entries(fn.chunks)) {
        for (const chunk of chunks) {
          expect(
            chunk.surfaceForms.length,
            `${fn.functionId}/${lang}/${chunk.chunkId} surfaceForms`
          ).toBeGreaterThan(0);
          expect(
            chunk.constituentLemmas.length,
            `${fn.functionId}/${lang}/${chunk.chunkId} constituentLemmas`
          ).toBeGreaterThan(0);
        }
      }
    }
...
describe("FunctionInventoryLoader", () => {
  ...
  it("getAllChunks returns chunks from all functions", () => {
    ...
    const allFunctionChunks = esInventory.functions.flatMap(
      (fn) => fn.chunks.es ?? []
    );
    expect(allChunks.length).toBe(allFunctionChunks.length);
  });
