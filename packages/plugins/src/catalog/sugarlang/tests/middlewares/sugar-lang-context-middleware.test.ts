...
describe("SugarLangContextMiddleware", () => {
  ...
  it("puts the form on a SCRIPTED turn, so an authored NPC can host the assessment", async () => {
    // The form used to be built only by sugaragent's free-form generate stage,
    // so an assessment on a scripted NPC rendered its intro as a dialogue line
    // and never produced a form. Nothing failed -- it just did not exist.
    ...
    const questionnaire = {
      ...
      supportLanguage: "en",
      formTitle: "Placement",
      formIntro: "Answer what you can in Spanish.",
    ...
    const execution = createTestExecution({
      ...
      state: { "sugaragent.session": { sessionId: "session-1", turnCount: 0, history: [] } }
    });
    ...
    const authoredTurn = {
      turnId: "turn-1",
      providerId: "engine.scripted-dialogue",
      conversationKind: "scripted-dialogue" as const,
      speakerId: "npc-1",
      speakerLabel: "Horace",
      displayName: "Hello",
    ...
    const finalized = await middleware.finalize?.(execution, authoredTurn as never);

    expect(finalized?.inputMode).toBe("quest_form");
    expect(finalized?.metadata?.["sugarlang.placementQuestionnaire"]).toEqual(questionnaire);
    // Still the provider's turn: the form rides on the authored line rather
    // than replacing the NPC's conversation.
    expect(finalized?.providerId).toBe("engine.scripted-dialogue");
  });
