 ...
 * (real GPU + WebGPU), loads postprocess-probe.html, waits for the render to settle,
...
const PORT = 5196;
const OUT = args.out ?? resolve(packageDir, "postprocess-probe.png");
const log = (...m) => console.log("[postprocess-probe]", ...m);
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

async function serverUp() {
  try {
    const r = await fetch(`http://localhost:${PORT}/postprocess-probe.html`, {
...
async function main() {
  ...
  try {
    ...
    const url =
      `http://localhost:${PORT}/postprocess-probe.html` + (args.preset ? "?preset" : "");
    log("launching real Chrome (binary + CDP, real GPU) ...");
    chrome = spawn(
      CHROME,
      [
        `--remote-debugging-port=${CDP_PORT}`,
        `--user-data-dir=${PROFILE}-ppprobe`,
        "--no-first-run",
    ...
    let page = ctx.pages().find((p) => p.url().includes("postprocess-probe.html"));
    if (!page) {
    ...
    const diag = await page.evaluate(() => globalThis.__probeDiag);
...
main().catch((e) => {
  console.error("[postprocess-probe] FAILED:", e.message);
  process.exit(1);
});
