...
**Locomotion decision (q9, 2026-07-17): `NavMeshQuery.computePath` + the existing manual stepper, NOT `Crowd`.** Justification (story-level per q9, no re-gate): our NPC locomotion is a per-NPC manual stepper that already OWNS the ECS `Position` and routes every move through the shared `resolveMove` (069.3 — the single collision enforcer, which already does circle-vs-circle avoidance + collide-and-slide). `Crowd` (dtCrowd) would seize position/velocity ownership and re-implement avoidance + clipping, creating TWO movement authorities fighting over the same body. computePath keeps ONE authority: the stepper heads to navmesh waypoints (routing around props), resolveMove stays the final clip, and inter-NPC avoidance is unchanged. Re-path triggers: target moved > 1m, route exhausted, drifted > 2.5m off the corner, or stuck (drops the route). Straight-line fallback when no pathfinder (unbaked regions).
