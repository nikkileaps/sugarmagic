...
**RESOLUTION (open question 3): DERIVE, not persist.** `ScatterGroup` is a derived view over `placedAssets` (`resolveScatterGroups`, domain) — no persisted entity, no migration, no aliases; old regions "have" groups the moment it ships and the round-trip is trivially invisible (region document unchanged). Layer 1 (derive + fixtures for folder-backed AND folderless buckets) committed 29df313. Layer 2 (its first live consumer — the Scene Explorer per-folder visibility eye): `resolveHiddenAssetInstanceIds` (domain, folder-subtree expansion) + `hiddenFolderIds` viewport state/projection + `authoringViewport.applyFolderVisibility` (ephemeral, never persisted). First cut hid instanced groups at the root (`.visible`), which was too coarse — a studio batch merges EVERY same-asset+surface instance across the whole scene into one InstancedMesh regardless of folder, so hiding a folder left same-asset siblings (in a shared batch) visible (nikki repro: hid a folder, only its lone-asset member vanished). FIX: per-instance hide — `buildInstancedAssetGroup` gains `setInstanceVisible(index, visible)` (collapses just that member to zero scale IN PLACE, so batch bounds barely move; idempotent so it's cheap to call for every member each projection), surfaced on `ReconciledEntry`; `applyFolderVisibility` hides per-member for groups, `.visible` for singletons. +1 reconciler test. Layer 3 (erase-per-group-membership) is already satisfied by the existing brush erase (removes `brushed` instances by folder); no new work.
