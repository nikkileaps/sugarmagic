...
**RESOLUTION (open question 3): DERIVE, not persist.** `ScatterGroup` is a derived view over `placedAssets` (`resolveScatterGroups`, domain) — no persisted entity, no migration, no aliases; old regions "have" groups the moment it ships and the round-trip is trivially invisible (region document unchanged). Layer 1 (derive + fixtures for folder-backed AND folderless buckets) committed 29df313. Layer 2 (its first live consumer — the Scene Explorer per-folder visibility eye, nikki-verified working): `resolveHiddenAssetInstanceIds` (domain, folder-subtree expansion, takes placed-assets/folders STRUCTURALLY) + `hiddenFolderIds` viewport state/projection. `authoringViewport` drops hidden-folder assets from the reconcile INPUT (they leave the scene; uniform for singletons and instanced batches — a shared batch just rebuilds without them), ephemeral and never persisted. THE BUG that ate two detours: the resolve must run over the SAME set the viewport renders — the COMPOSED base + active-Scene overlay (`composeRegionContents`), NOT base `region.placedAssets` alone. With base-only, scene-scoped placements in a hidden folder rendered but never hid (nikki repro: a base rock hid, an overlay bench/tree in the same folder did not — which also disproved the instancing/batching red herrings I chased first). Layer 3 (erase-per-group-membership) is already satisfied by the existing brush erase (removes `brushed` instances by folder); no new work.
...
### 070.5 — Instanced mask sampling fix (#360) — DONE (2026-07-18), probe-verified on real GPU

**PROBE RESULT (the verification I owed — `pnpm --filter @sugarmagic/perf-harness probe:mask`, a durable committed probe, NOT a throwaway):** instanced boxes at x=0/6/12, colored by a local-X gradient two ways. `positionLocal` (the old code): x=0 ramps but x=6 & x=12 render FLAT (saturated) — the #360 bug reproduced. `positionGeometry` (the fix): all three ramp correctly. Confirms the mechanism on hardware, not just from source.

**Process note (nikki pushback):** the plan said PROBE FIRST and I skipped it, rationalizing that the epic review had "source-confirmed" the mechanism — which missed that a vertical gradient on a ground-standing object works ANYWAY (world-Y ≈ local-Y coincidence), so the bug only visibly bites horizontal / off-origin gradients. Reading source is not verifying on GPU. Logged as a durable lesson.



**Findings / resolution:** the mechanism was already source-confirmed across review rounds 1-2 (installed three 0.183.2): `InstanceNode`/`BatchNode` reassign `positionLocal` to the instance-world position BEFORE user nodes, so `materialize/mask.ts` normalizing `positionLocal` against the geometry's object-space bounds saturated -> flat. A separate loud-value probe would only re-confirm the source read, so the fix went straight in: the local-space branch of `gradientAxisNode` now samples the RAW `positionGeometry` attribute instead of `positionLocal`. Rigid non-instanced meshes are unaffected (`positionLocal === positionGeometry` there); instanced assets now get per-geometry local ramps; deformed assets ramp bind-pose-relative (the intended "local" meaning). render-web typechecks. **Still needs the visual repro-verify** (two instanced statues + a local gradient mask -> shade per-geometry, not flat; a world-space height mask still varies across a field; non-instanced rigid assets pixel-identical).

### 070.5 — original (locked) story text — PROBE FIRST
...
### 070.6 — Studio viewport adopts instancing (#348) — DONE (2026-07-18, live-verified: brushed a meadow, click-selected one plant, moved it, only that one moved; plants batched). Also caught+fixed+guarded a 070.2 marker regression (studio picking/painting). Deferred refinement: per-member hover hull for instanced selections.
...
### 070.7 — Preview boot-once (#345b) — RE-SCOPED from "live-resync" to a bug fix, pending nikki verify

**WHY re-scoped (nikki, 2026-07-19):** "what actual problem are we solving?" broke this open. The locked framing ("prove 070.2 on the game host" via live-resync) was a testing motivation, not a problem — and 070.2 is already proven (studio viewport 070.6, game item-removal 070.2). The ONE real problem: the Preview **randomly bounces you back to the Start menu** ("hit New Game, get in, it reboots and shows New Game again"). Mechanism: the PREVIEW_BOOT effect (App.tsx) re-posted on `selectedIds` / workspace-tab / `session`-edit churn; `host.start()` isn't idempotent (preview.tsx tears down + reboots) and the fresh-start flag is consumed on the first boot, so any re-post reboots WITHOUT `skipStartMenuOnBoot` → Start menu. The `assetSources` re-post was patched before; this generalizes the fix.

**Live-resync (old Layer 2) CUT.** Prior art: Unreal PIE is an isolated level copy — editor edits don't propagate into the running play session ([docs](https://dev.epicgames.com/documentation/en-us/unreal-engine/playing-and-simulating-in-unreal-engine)); the editor viewport (their SIE / our Layout+Spatial viewports, already live via 070.3/070.6) is the live-edit surface. nikki confirmed she doesn't want a live preview ("I close the preview then go do work in studio and use the workspace viewports"). So the Preview boots ONCE on launch and never auto-reboots on Studio churn; re-launch to pick up edits. FIX: the boot effect reads every payload value imperatively and triggers only on `[isPreviewRunning, previewWindow, hasSession]`.
**Verify:** launch Preview → New Game → in the game; then select objects / switch tabs / edit an asset in Studio → the game STAYS put (no Start-menu bounce, player position preserved); close + re-launch Preview → your edits are there.

### 070.7 — original (locked) story text — live-resync
...
### 070.8 — Epic close (#344) — DONE (2026-07-19), trimmed scope by value (nikki-approved)

**Shipped:** `reconciler.stats()` ({singletons, groups, instances, drawUnits}) + `ShaderRuntime.getMaterialStats()` (material cache was private) + a NO-GPU headless budget alarm (`packages/testing/src/render-budget.test.ts`): a fixture meadow (250 shared-surface plants + distinct props) folds to a handful of draw units under budget, and a regression case (surfaces stop sharing a `representationKey`) trips it. docs/api rewritten true to code (the shared reconciler + instancing + scatter groups + folder-eye replaced the stale generation-number section). Deleted the dead `renderable-lifecycle.ts` + its test (070.2 replaced it; only a barrel line + self-test still referenced it — bias-to-deletion sweep).

**Trimmed (nikki-approved, value-scoped):**
- **`lastPreparedFrameByCamera` "leak" — NOT a leak, no fix.** `scatter/compute-pipeline.ts` keys it by `camera.uuid` (a game has ~1 camera) and already `.clear()`s it on reset. Bounded by distinct-camera count, which is tiny. Dismissed with proof, not patched.
- **Perf before/after harness run — trimmed to this honest note.** The epic's fps premise (070.4) was DEFERRED once 070.1 showed the bottleneck is the BASE scene render (~22ms GPU), not scatter/shadows. So 070 delivered correctness + structural unification (one reconciler across both hosts, instancing in the studio, the #360 mask fix, derived scatter groups, the preview boot-once fix) — NOT a measured fps win. The real fps work lives in the deferred base-render hunt + 070.4 re-scope.

**Deferred-seam sweep (durable trackers):** scatter shadows / HISM culling -> #347; base-render ~22ms perf hunt -> new backlog task; linked/unlinked dup -> #361; per-member hover hull for instanced selection (070.6) + overlay-scope scatter membership (070.3) -> documented in code + this plan (low priority, no task).

### 070.8 — original (locked) story text
