...
**Relationship to the EXISTING authored grouping (review round 1):** the brush ALREADY mints a "<Asset> patch" folder per arm-session (`scatter-brush.ts` `resolveSessionFolder` + `BrushPlaceAssets.createFolder` in the same transaction) — 070.3 must not build a second grouping axis beside it. Decision (refine in-story): `ScatterGroup` becomes the authored grouping for brushed content; **patch folders are the migration provenance** (existing brushed members group by their patch-folder `parentFolderId`); whether patch folders remain as pure organization or are retired for brushed content is decided in-story. ALSO: `applyBrushPlaceAssets` supports scope "scene" (writes into Scene overlays) even though the tool currently hardcodes base — the group model + migration must define overlay-scoped membership or explicitly exclude it.

Normalized on load; existing `brushed: true` loose instances migrate into groups (069.4 discipline: invisible round-trip, aliases if needed). Studio: groups appear in the scene explorer (visibility eye per group).
**Verify:** brush a stroke -> ONE group with N members in the document; old regions load with brushed plants grouped BY their patch folders — byte-identical document round-trip + pixel-identical screenshot (scatter render harness recipe); group eye hides the whole stroke; erase respects group membership; overlay-scope behavior is defined and tested (or explicitly excluded).
...
**Verify:** the probe's findings recorded in this doc; two scattered statues with a local gradient mask shade per their own geometry (the 2026-07-16 repro); a world-space height mask varies correctly across a field; non-instanced assets pixel-identical before/after; material count unchanged (no cache forking).
