...
- **Filters object shape**: single `shouldSkip` predicate. Future filters compose into the same predicate. This is intentionally the "boring" shape — if we need multiple named filters later (Part, collected, unlock-schedule) we can widen the object without changing callers.
