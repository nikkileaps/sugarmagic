...
- **`world.presence` scope when Parts ships (Plan 058).** Current slice is `Record<regionId, string[]>` — Part-agnostic. If a later Part needs to un-collect a presence in a shared region to re-story it, that has to be handled at authoring time or via a runtime override rule. If that pattern gets common we bump `world.presence` schemaVersion to `Record<regionId, Record<partId, string[]>>` and migrate. Not blocking 055; revisit when 058 defines the Part content-load contract.
