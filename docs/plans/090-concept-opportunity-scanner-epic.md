...
- **`chunks` -> `exponents` on a competency -- DONE (sugarmagic-lessons-222.3). The trigger fired when the curriculum put chunk at the leaf of a tree the competency path had to name differently. `Competency.exponents` holds `Exponent` with `exponentId`; `LexicalChunk` keeps `chunkId` on the MWE path. They were untangled by making the matcher generic over `normalizedForm`, which is the join key both already had, so neither borrows the other's id. Learner cards are keyed `exponent:<id>`.**
