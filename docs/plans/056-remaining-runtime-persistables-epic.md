...
Related: Plan 055 (SaveParticipant model — the mechanism this epic uses). Plan 058 (Parts content model draft — some of these participants intersect with Part gating in obvious ways). ADR 020 (per-plugin per-user state boundary — participants here are runtime-core system state, not plugin data).
...
- **Parts progression** — Plan 058's `campaign.progression` participant. Separate epic; different design surface.
...
- **`campaign.progression` doubling as a mode for participant "reset when Part changes."** Cross-participant reset semantics are Plan 058's problem.
...
Wire: `NpcBehaviorSystem` gains `serializeSaveSlice()` / `deserializeSaveSlice(slice)` methods that read and write its private `movementStateByNpcId` map + query/update the ECS `Position` component for each NPC entity. Participant factory reads it via getter. Tier: `default` (Phase 2) — NPC entities are created inside `gameplayAssembly` at construction time (spawn-point position), then the participant's deserialize overwrites the Position component with the persisted value. No visual flicker because Phase 2 runs before the first render frame; the first tick sees the restored position and computes movement from there.
