...
## World flags on the blackboard

`WorldFlagManager` owns world flag values and saves them in its own slice. The
blackboard carries a read copy so a system can answer "is this flag set" without
holding the store.

**One definition, one fact per flag.** `WORLD_FLAG_FACT` has the key
`world.flag` and is owned by `world-flag-system`. Flags are told apart by the
scope id, which is the flag's **name** -- the same key the store uses, so a
console readout and a blackboard lookup name the same thing. Read with
`getWorldFlagFact(blackboard, name)`; it returns `null` for a flag that is unset
or unregistered. The lifecycle is `session`: the facts clear with the session
and are re-published from the save slice on restore.

**One write path.** Every value that enters the flag store goes through a single
private method on `WorldFlagManager`, and everything that leaves goes through
its pair. `createWorldFlagProjection` (`world-flags/projection.ts`) attaches to
that as a write observer and is wired in `gameplay-session.ts`. This is
deliberately separate from the store's change handler: the change handler means
"re-evaluate quest conditions" and is skipped for the write that runs inside the
quest refresh loop, which is the path every authored `setFlag` action takes. An
observer sharing that skip would miss the most common write in the game.

**Only registered flags are projected.** The project's flag registry
(`GameProject.worldFlagDefinitions`) is what makes the projected keys a closed
set rather than whatever string a caller passed. Two callers can still write a
name the registry does not list -- the `__smsetflag` console handle and an
agent's conversation proposal. Those flags are set and saved as normal, are not
readable on the blackboard, and log one warning naming the flag.
