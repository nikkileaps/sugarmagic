...
- [Telemetry](/Users/nikki/projects/sugarmagic/docs/api/sugarlang-telemetry.md) -- the shared collector in runtime-core, event taxonomy + schema versioning, sugaragent's degraded-turn event, the `/api/telemetry` ingestion route, client-side PII scrub, proxy base URL env resolution
